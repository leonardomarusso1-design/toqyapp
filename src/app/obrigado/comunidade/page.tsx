...
export default function ObrigadoComunidade() {
  ...
  useEffect(() => {
    async function check() {
      ...
      if (!session) { router.replace("/login?next=/obrigado/comunidade"); return; }

      const { data: profile } = await supabase
        .from("profiles")
        .select("plan_toqy, subscription_status")
        .eq("id", session.user.id)
        .maybeSingle();

      const plan = profile?.plan_toqy ?? "free";
  ...
  if (status === "loading") return (
    <main className="flex min-h-screen items-center justify-center bg-[#0d1117]">
      <div className="h-10 w-10 animate-spin rounded-full border-4 border-[#31c4a8] border-t-transparent" />
    </main>
  );
  ...
  if (status === "unauthorized") return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-[#0d1117] px-5 text-center text-white">
      ...
      <p className="mt-4 max-w-md text-slate-400">Esta página é exclusiva para membros com plano ativo. Adquira o plano Comunidade para ter acesso ao Discord e a todos os recursos.</p>
      <a href="https://pay.kiwify.com.br/12uYE0c" className="mt-8 inline-flex rounded-2xl bg-[#31c4a8] px-8 py-4 font-black text-white transition hover:bg-[#25b69a]">
        Assinar Comunidade — R$29,90/mês
      </a>
      <Link href="/" className="mt-4 text-sm text-slate-500 hover:text-slate-300">Voltar para o início</Link>
    </main>
  );
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-[#0d1117] px-5 text-center text-white">
      ...
      <div className="mt-6 w-full max-w-md rounded-[2rem] border border-[#31c4a8]/20 bg-[#31c4a8]/10 p-6">
        <p className="font-black text-[#31c4a8]">Próximo passo</p>
      ...
      <p className="mt-10 text-xs text-slate-600">Dúvidas? Fale no Discord ou envie um e-mail para leonardomarusso1@gmail.com</p>
    </main>
  );
}
