...
import { syncBiositeToSupabase, loadBiositeFromSupabase } from "@/lib/biositeSync";
...
function EditPageInner({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    params.then(async ({ slug }) => {
      ...
      let found = await loadBiositeFromSupabase(slug);

      // Fallback: localStorage
      if (!found) found = getSiteBySlug(slug) ?? null;

      setSite(found);
      setLoading(false);

      // Auto-unlock se chave vier na URL
  ...
  async function verifyKey() {
    if (!site) return;
    // Verificar chave contra Supabase (mais seguro)
  ...
  async function handleSave(updated: ToqySite) {
    setSaving(true);
    try {
      await syncBiositeToSupabase(updated);
      setSite(updated);
    } finally {
      setSaving(false);
    }
  ...
  if (loading) return (
    <ClientShell>
      <div className="mx-auto max-w-md rounded-[2rem] border border-slate-100 bg-white p-8 text-center shadow-sm">
        <div className="mx-auto h-8 w-8 animate-spin rounded-full border-4 border-[#31c4a8] border-t-transparent" />
        <p className="mt-4 text-sm font-bold text-slate-500">Carregando...</p>
      </div>
    </ClientShell>
  );
