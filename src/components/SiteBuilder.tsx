...
const field = "mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100";
  ...
  const body = (() => {
      ...
      return (
        <Section>
          ...
          <div className="mt-5 rounded-3xl border border-blue-100 bg-blue-50 p-4">
            <p className="font-black text-blue-950">{selectedTemplate.templateName}</p>
            <p className="mt-1 text-sm text-blue-800">{selectedTemplate.description}</p>
            <p className="mt-2 text-xs font-bold text-blue-700">Layout de catálogo sugerido: {selectedTemplate.catalogLayout ?? "carousel"}</p>
            <button type="button" onClick={() => update((s) => applySegmentTemplate(s, site.segment))} className="mt-4 rounded-2xl bg-blue-600 px-5 py-3 text-sm font-black text-white">Aplicar template deste segmento</button>
          </div>
        </Section>
      );
    }
        ...
        <Section>
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-2">
            ...
            <div className="rounded-3xl border border-blue-100 bg-blue-50 p-4">
              <h3 className="font-black text-blue-950">Wi‑Fi + check-in</h3>
              <div className="mt-4 grid gap-4">
      ...
      return (
        <Section>
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            ...
            <button type="button" onClick={() => update((s) => ({ ...s, catalog: [...s.catalog, { id: generateId("prd"), name: "Novo item", description: "Descrição do item", price: "", imageUrl: "", imageLayout: "square", category: "Destaques", enabled: true, actionLabel: "Ver detalhes", actionUrl: "" }] }))} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-4 py-3 text-sm font-black text-white"><Plus className="h-4 w-4" />Adicionar</button>
          </div>
          <div className="mt-5 rounded-3xl border border-slate-200 bg-slate-50 p-4">
            <label><span className={label}>Como exibir no bio site</span><select className={field} value={site.catalogLayout ?? "carousel"} onChange={(e) => update((s) => ({ ...s, catalogLayout: e.target.value as CatalogLayout }))}><option value="carousel">Carrossel único</option><option value="grid">Grade 2 colunas</option><option value="stack">Imagem uma embaixo da outra</option><option value="grouped">Carrossel por categoria</option></select></label>
    ...
    return (
      <Section>
        ...
        <button type="button" onClick={save} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-blue-600 px-5 py-4 font-black text-white"><Save className="h-5 w-5" />Salvar e publicar</button>
      ...
      <div className="min-w-0">
        <div className="mb-5 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm md:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div><p className="text-sm font-black uppercase tracking-[0.18em] text-blue-600">TOQY Builder</p><h1 className="mt-2 text-3xl font-black text-slate-950 md:text-5xl">{mode === "create" ? "Criar bio site" : "Editar bio site"}</h1><p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-500">Tudo editável com preview ao vivo. Depois entregue link, QR Code e chave para o cliente.</p></div>
            <button type="button" onClick={save} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-5 py-3 text-sm font-black text-white"><Save className="h-4 w-4" />Salvar agora</button>
          </div>
        ...
        <div className="mb-5 flex gap-2 overflow-x-auto rounded-[1.5rem] border border-slate-200 bg-white p-2 shadow-sm">{steps.map((item, index) => <button key={item} type="button" onClick={() => setStep(index)} className={`shrink-0 rounded-2xl px-4 py-2 text-sm font-black transition ${index === step ? "bg-blue-600 text-white" : "text-slate-600 hover:bg-slate-100"}`}>{index + 1}. {item}</button>)}</div>
        {body}
        <div className="mt-5 flex flex-col gap-3 rounded-[1.5rem] border border-slate-200 bg-white p-3 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-3"><button type="button" onClick={save} className="rounded-2xl border border-blue-200 bg-blue-50 px-5 py-3 text-sm font-black text-blue-700">Salvar agora</button><button type="button" onClick={() => step < steps.length - 1 ? setStep((v) => v + 1) : save()} className="rounded-2xl bg-blue-600 px-5 py-3 text-sm font-black text-white">{step < steps.length - 1 ? "Continuar" : "Salvar e publicar"}</button></div>
        </div>
      </div>
