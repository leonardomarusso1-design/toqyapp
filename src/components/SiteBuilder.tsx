...
const steps = ["Modelo", "Perfil", "Visual", "Links e BotÃµes", "Pix e Wi-Fi", "CatÃ¡logo", "Salvar"];
...
export function SiteBuilder({ mode, initialSite, onSave }: Props) {
  ...
  const body = (() => {
    if (step === 0) {
      return (
        <Section>
          ...
          <p className="mt-1 text-sm text-slate-500">Escolha o segmento e aplique um template realmente diferente: cores, botÃµes, catÃ¡logo, categorias e layout.</p>
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            ...
            <label><span className={label}>Nome do negÃ³cio</span><input className={field} value={site.profile.name} onChange={(e) => update((s) => ({ ...s, profile: { ...s.profile, name: e.target.value }, slug: s.slug === "novo-negocio" || !s.slug ? generateSlug(e.target.value) : s.slug }))} /></label>
            <label><span className={label}>Link da pÃ¡gina</span><input className={field} value={site.slug} onChange={(e) => update((s) => ({ ...s, slug: generateSlug(e.target.value) }))} /></label>
          </div>
          <div className="mt-5 rounded-3xl border border-emerald-100 bg-emerald-50 p-4">
            ...
            <p className="mt-2 text-xs font-bold text-emerald-700">Layout de catÃ¡logo sugerido: {selectedTemplate.catalogLayout ?? "carousel"}</p>
      ...
      return (
        <Section>
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <label><span className={label}>Tamanho da logo</span><select className={field} value={site.profile.logoSize} onChange={(e) => setProfile({ logoSize: e.target.value as ToqySite["profile"]["logoSize"] })}><option value="small">Pequena</option><option value="medium">MÃ©dia</option><option value="large">Grande</option></select></label>
            ...
            <label><span className={label}>TÃ­tulo/subtÃ­tulo</span><input className={field} value={site.profile.title ?? ""} onChange={(e) => setProfile({ title: e.target.value })} /></label>
            <label><span className={label}>LocalizaÃ§Ã£o</span><input className={field} value={site.profile.location} onChange={(e) => setProfile({ location: e.target.value })} /></label>
            <label className="md:col-span-2"><span className={label}>DescriÃ§Ã£o</span><textarea className={field} rows={3} value={site.profile.description} onChange={(e) => setProfile({ description: e.target.value })} /></label>
    ...
    if (step === 2) {
      return (
        <Section>
          ...
          <p className="mt-1 text-sm text-slate-500">Paletas, fundo da plaquinha e botÃµes premium inspirados no modelo enviado.</p>
          <div className="mt-5"><ThemePresetPicker selectedPresetId={site.themePresetId} onSelect={selectTheme} /></div>
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            <label><span className={label}>Tipo de fundo</span><select className={field} value={site.theme.backgroundType} onChange={(e) => setTheme({ backgroundType: e.target.value as ToqySite["theme"]["backgroundType"] })}><option value="gradient">Gradiente</option><option value="solid">Cor sÃ³lida</option><option value="image">Imagem</option></select></label>
            <label><span className={label}>Layout dos botÃµes</span><select className={field} value={site.theme.buttonStyle} onChange={(e) => setTheme({ buttonStyle: e.target.value as ToqySite["theme"]["buttonStyle"] })}><option value="full">BotÃµes grandes</option><option value="icon">Grade de Ã­cones</option></select></label>
            <label><span className={label}>Preenchimento</span><select className={field} value={site.theme.buttonFill} onChange={(e) => setTheme({ buttonFill: e.target.value as ToqySite["theme"]["buttonFill"] })}><option value="glass">TranslÃºcido premium</option><option value="solid">SÃ³lido</option><option value="gradient">Gradiente</option></select></label>
            <label><span className={label}>Formato</span><select className={field} value={site.theme.buttonRadius} onChange={(e) => setTheme({ buttonRadius: e.target.value as ToqySite["theme"]["buttonRadius"] })}><option value="soft">Soft</option><option value="rounded">Arredondado</option><option value="pill">Pill/cÃ¡psula</option></select></label>
          ...
          <button type="button" onClick={() => setAdvancedColors((v) => !v)} className="mt-5 rounded-2xl border border-slate-200 px-4 py-3 text-sm font-black text-slate-700">{advancedColors ? "Ocultar cores avanÃ§adas" : "Personalizar cores avanÃ§adas"}</button>
    ...
    if (step === 4) {
      return (
        <Section>
          <h2 className="text-2xl font-black text-slate-950">Pix e Wiâ€‘Fi</h2>
          <p className="mt-1 text-sm text-slate-500">Configure Pix com comprovante e Wiâ€‘Fi com check-in/avaliaÃ§Ã£o.</p>
          <div className="mt-5 grid gap-5 lg:grid-cols-2">
            <div className="rounded-3xl border border-emerald-100 bg-emerald-50 p-4">
              ...
              <div className="mt-4 grid gap-4">
                ...
                <label><span className={label}>Banco/observaÃ§Ã£o</span><input className={field} value={site.pix.bank ?? ""} onChange={(e) => update((s) => ({ ...s, pix: { ...s.pix, bank: e.target.value } }))} /></label>
                ...
                <label><span className={label}>Valores rÃ¡pidos</span><input className={field} value={site.pix.quickAmounts.join(", ")} onChange={(e) => update((s) => ({ ...s, pix: { ...s.pix, quickAmounts: e.target.value.split(",").map((v) => Number(v.trim())).filter(Boolean) } }))} /><Help>Exemplo: 10, 20, 50. Eles aparecem como botÃµes rÃ¡pidos no modal Pix.</Help></label>
              </div>
            </div>
            <div className="rounded-3xl border border-emerald-100 bg-emerald-50 p-4">
              <h3 className="font-black text-emerald-950">Wiâ€‘Fi + check-in</h3>
              <div className="mt-4 grid gap-4">
                <label><span className={label}>Nome da rede Wiâ€‘Fi</span><input className={field} value={site.wifi.ssid} onChange={(e) => update((s) => ({ ...s, wifi: { ...s.wifi, ssid: e.target.value, enabled: true } }))} /></label>
                <label><span className={label}>Senha Wiâ€‘Fi</span><input className={field} value={site.wifi.password} onChange={(e) => update((s) => ({ ...s, wifi: { ...s.wifi, password: e.target.value } }))} /></label>
                <label><span className={label}>SeguranÃ§a</span><select className={field} value={site.wifi.encryption} onChange={(e) => update((s) => ({ ...s, wifi: { ...s.wifi, encryption: e.target.value as ToqySite["wifi"]["encryption"] } }))}><option value="WPA">WPA/WPA2</option><option value="WEP">WEP</option><option value="nopass">Sem senha</option></select></label>
                <label><span className={label}>Link de check-in/avaliaÃ§Ã£o</span><input className={field} value={site.wifi.checkinUrl ?? ""} onChange={(e) => update((s) => ({ ...s, wifi: { ...s.wifi, checkinUrl: e.target.value } }))} placeholder="Facebook, Google avaliaÃ§Ã£o, Instagram..." /></label>
                <label><span className={label}>Texto do botÃ£o de check-in</span><input className={field} value={site.wifi.checkinLabel ?? ""} onChange={(e) => update((s) => ({ ...s, wifi: { ...s.wifi, checkinLabel: e.target.value } }))} placeholder="Fazer check-in no Facebook" /></label>
                <label><span className={label}>Link Google avaliaÃ§Ã£o</span><input className={field} value={site.links.googleReviewUrl ?? ""} onChange={(e) => setLinks({ googleReviewUrl: e.target.value })} /></label>
              </div>
            </div>
          </div>
        </Section>
      ...
      return (
        <Section>
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
              <h2 className="text-2xl font-black text-slate-950">CatÃ¡logo</h2>
              <p className="mt-1 text-sm text-slate-500">Escolha entre carrossel, grade, lista vertical ou carrossÃ©is por categoria.</p>
            </div>
          ...
          <div className="mt-5 rounded-3xl border border-slate-200 bg-slate-50 p-4">
            <label><span className={label}>Como exibir no bio site</span><select className={field} value={site.catalogLayout ?? "carousel"} onChange={(e) => update((s) => ({ ...s, catalogLayout: e.target.value as CatalogLayout }))}><option value="carousel">Carrossel Ãºnico</option><option value="grid">Grade 2 colunas</option><option value="stack">Imagem uma embaixo da outra</option><option value="grouped">Carrossel por categoria</option></select></label>
            <Help>Use â€œcarrossel por categoriaâ€ para exemplos como: Cortes social, Cortes degradÃª, Barba e tratamentos.</Help>
          </div>
            ...
            {site.catalog.map((item, index) => (
              <article key={item.id} className="rounded-3xl border border-slate-200 p-4">
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <label><span className={label}>PreÃ§o</span><input className={field} value={item.price ?? ""} onChange={(e) => update((s) => ({ ...s, catalog: updateCatalogItem(s.catalog, index, { price: e.target.value }) }))} /></label>
                  ...
                  <label className="md:col-span-2"><span className={label}>DescriÃ§Ã£o</span><textarea className={field} rows={2} value={item.description} onChange={(e) => update((s) => ({ ...s, catalog: updateCatalogItem(s.catalog, index, { description: e.target.value }) }))} /></label>
                </div>
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-[1fr_1fr_auto_auto]">
                  <input className={field} placeholder="Texto do botÃ£o" value={item.actionLabel ?? ""} onChange={(e) => update((s) => ({ ...s, catalog: updateCatalogItem(s.catalog, index, { actionLabel: e.target.value }) }))} />
                  <input className={field} placeholder="Link do botÃ£o" value={item.actionUrl ?? ""} onChange={(e) => update((s) => ({ ...s, catalog: updateCatalogItem(s.catalog, index, { actionUrl: e.target.value }) }))} />
    ...
    return (
      <Section>
        <h2 className="text-2xl font-black text-slate-950">{mode === "create" ? "Publicar" : "Salvar alteraÃ§Ãµes"}</h2>
